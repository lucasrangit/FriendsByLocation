...
class FriendsPage(BaseHandler):
  ...
  def get(self):
    ...
    if userprefs:
      ...
      friends_of_friends = graph.fql("SELECT uid, name, profile_url, current_location FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1 IN (SELECT uid FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1 = me()) AND is_app_user=1)) AND " + str(userprefs.location_id) + " in current_location.id AND NOT (uid=me())")
